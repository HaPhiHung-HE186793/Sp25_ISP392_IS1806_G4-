...
public class createSchedule extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ( roleID != 2) {
            ...
            response.sendRedirect("liststore"); // sửa thành đường dẫn của trang chủ sau khi hoàn thành code
            return;
        }
...
// Kiểm tra statusSelect
        ...
        if (!errors.isEmpty()) {
            ...
            List<Schedule> schedules = daos.listSchedule(user_current.getStoreID());
        ...
        Schedule schedule = new Schedule(shiftName, shiftStart, shiftEnd, breakStart, breakEnd, isDelete,user_current.getID(), user_current.getStoreID());
        daos.createSchedule(schedule);

        List<Schedule> schedules = daos.listSchedule(user_current.getStoreID());

        // Cập nhật pagination dựa trên số lượng kết quả tìm kiếm
        int totalSchedule = schedules.size();
        int pageSize = 9;
        int currentPage = 1;
