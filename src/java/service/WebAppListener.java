...
public class WebAppListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        debtChecker.submit(() -> {
            while (true) {
                try {
                    List<DebtRecords> unprocessedDebts = dao.getUnprocessedDebts(); // Lấy bản ghi chưa xử lý
    ...
    private void processDebtRecord(DebtRecords record, DAODebtRecords dao) {
        ...
        try {
            dao.db.conn.setAutoCommit(false); // Bắt đầu transaction

            // Cập nhật nợ khách hàng
            dao.updateCustomerDebt(record.getCustomerID(), record.getPaymentStatus(), record.getAmount());

            // Đánh dấu là đã xử lý trong bảng DebtRecords
            dao.markDebtAsProcessed(record.getDebtID());

            dao.db.conn.commit(); // Commit nếu thành công

        } catch (SQLException e) {
            try {
                dao.db.conn.rollback(); // Rollback nếu lỗi
